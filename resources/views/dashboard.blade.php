        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                <div class="bg-slate-800 p-4 rounded-lg border border-slate-700 shadow-lg text-center">
                    ...
                    <div class="text-xs text-slate-400 uppercase tracking-widest">Jornadas Finalizadas</div>
                </div>
                ...
                <div class="bg-slate-800 p-4 rounded-lg border border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.1)] text-center relative overflow-hidden">
                    ...
                    <div class="text-xs text-amber-600 uppercase tracking-widest">Lendas Forjadas</div>
                </div>
                ...
                <div class="bg-slate-800 p-4 rounded-lg border border-red-900/50 shadow-lg text-center relative overflow-hidden">
                    ...
                    <div class="text-xs text-red-900/70 uppercase tracking-widest">Caídos em Combate</div>
                </div>

                <div class="bg-slate-800 p-4 rounded-lg border border-slate-700 shadow-lg text-center">
                    <div class="text-3xl mb-1">📈</div>
                    <div class="text-2xl font-bold text-blue-300 font-cinzel">{{ $winRate }}%</div>
                    <div class="text-xs text-slate-400 uppercase tracking-widest">Taxa de Vitória</div>
                </div>
            </div>
            ...
            @if(is_null($heroes))
                <div class="text-center py-20 bg-slate-800/50 rounded-xl border border-dashed border-slate-700">
                    <p class="text-slate-500 font-cinzel text-xl">O salão está vazio.</p>
                    <p class="text-slate-600 text-sm mt-2">Complete sua primeira jornada para ser imortalizado aqui.</p>
                </div>
            @else
                <h3 class="font-cinzel text-slate-300 text-lg mb-6 flex items-center gap-2">
                    <span class="w-2 h-2 bg-amber-500 rounded-full"></span> Registros Antigos
                </h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($heroes as $hero)
                        @php
                            ...
                            $icon = $isWin ? '👑' : '💀';
                            $statusText = $isWin ? 'Ascendeu como Lenda' : 'Pereceu na Escuridão';
                        ...
                        <div class="relative bg-slate-800 border {{ $borderColor }} rounded-xl overflow-hidden {{ $glow }} transition-all duration-300 flex flex-col group {{ $opacity }}">
                            ...
                            <div class="p-4 bg-slate-900/50 border-b border-slate-700 flex justify-between items-center">
                                <span class="font-cinzel font-bold text-slate-200">Herói #{{ $hero->getId() }}</span>
                                <span class="text-2xl" title="{{ $statusText }}">{{ $icon }}</span>
                            </div>

                            <div class="p-6 space-y-4">
                                <div class="text-center mb-4">
                                    <p class="text-xs font-bold {{ $statusColor }} uppercase tracking-widest mb-1">{{ $statusText }}</p>
                                    <p class="text-xs text-slate-500">Terminou no Capítulo {{ $hero->getCurrentStoryNode() }}</p>
                                </div>
                                ...
                                <div class="grid grid-cols-3 gap-2 text-center text-sm bg-slate-900 rounded p-3 border border-slate-700/50">
                                    <div>
                                        ...
                                        <div class="text-[10px] text-slate-600 uppercase">Hab</div>
                                    </div>
                                    <div>
                                        <div class="text-red-500 font-bold">{{ $hero->getEnergyCurrent() }}</div>
                                        <div class="text-[10px] text-slate-600 uppercase">Ene</div>
                                    </div>
                                    <div>
                                        <div class="text-green-500 font-bold">{{ $hero->getLuckCurrent() }}</div>
                                        <div class="text-[10px] text-slate-600 uppercase">Sor</div>
                                    </div>
                                </div>

                                <div class="flex justify-between items-center px-2 mt-2">
                                    <span class="text-xs text-slate-500 uppercase">Fortuna deixada</span>
                                    <span class="text-amber-400 font-mono font-bold">{{ $hero->getGold() }} Ouro</span>
                                </div>
                            </div>

                            <div class="px-6 py-3 bg-slate-950/30 text-right">
                                <span class="text-[10px] text-slate-600">
                                    Registrado em: {{ $hero->updated_at->format('d/m/Y') }}
                                </span>
                            </div>
                        </div>
                    @endforeach
